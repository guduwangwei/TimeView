package com.ww.timeviewlibrary;
...
//    private OnScaleListener mListener;
    ...
    public RulerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTextColor = ta.getColor(R.styleable.RulerView_textColor_rule, Color.GRAY);
        mTextSize = ta.getDimension(R.styleable.RulerView_textSize_rule, 18);
        mBeginRange = ta.getInt(R.styleable.RulerView_begin, 0);
        mEndRange = ta.getInt(R.styleable.RulerView_end, 24);
        // 刻度的宽度
    ...
    public void setSelectInf(SelectInf selectInf) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setColor(Color.parseColor("#e63939"));
        paint.setStyle(Paint.Style.FILL);
        paint.setStrokeWidth(7);
        canvas.drawLine(halfWidth + getScrollX(), 0, halfWidth + getScrollX(), 150, paint);
    ...
    private void drawText(Canvas canvas, int position, String text) {
        ...
        canvas.drawText(String.valueOf(Integer.parseInt(text) / 6), x, y, mTextPaint);
    }
...
//        }
    ...
    private int getIndicateWidth() {
        return mIndicateWidth + mIndicatePadding;
    }

    private int getsmallItemWidth() {
        return mIndicatePadding + mIndicateWidth;
    }
    ...
    private int getMinimumScroll() {
        return -getWidth() / 2 + getStartOffsets();
    }
...
//        return scrollX;
//    }
    public void setCurrent(int hour, int minite) {
        //屏幕的半个宽度
        this.currentHour = hour;
        this.currentMini = minite;
        int lPox = getIndicateWidth() / 10 * currentMini;
        int posX = currentHour * 6 * getIndicateWidth() - halfWidth + lPox;
        mOverScroller.startScroll(getScrollX(), getScrollY(), posX, 250);
        invalidateView();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (w != oldw) {
            halfWidth = w / 2;
            setCurrent(currentHour, currentMini);
            invalidateView();
        }
    }
...
//        }
    ...
    public void computeScroll() {
        if (mOverScroller.computeScrollOffset()) {
            ...
            Log.v("whellView", "stop--" + (mOverScroller.getFinalX() - lastX) + "--" + mOverScroller.getFinalX());

            if (!frist) {
                float fx = mOverScroller.getFinalX();
                int dx = 0;
                if ((lastX > 0 && fx < 0) || (lastX < 0 && fx > 0)) {
                    dx = (int) (Math.abs(fx) + Math.abs(lastX));
                } else {
                    dx = (int) Math.abs((fx - lastX));
                }
                if (fx > lastX) {
                    int num = (dx * 10) / getIndicateWidth();
                    int mini = (num) % 60;
                    int hour = (int) Math.floor(num / 60);
                    cHour = hour + currentHour;
                    cMini = mini + currentMini;
                    if (cMini >= 60) {
                        cMini = (cMini) % 60;
                        cHour = cHour + 1;
                    }
                    ...
                    int mini = (num) % 60;
                    int hour = (int) Math.floor(num / 60);
                    int dexMi = currentMini - mini;
                    cHour = currentHour - hour;
                    cMini = dexMi;
                    if (dexMi < 0) {
                        cHour = cHour - 1;
                        cMini = 60 + dexMi;
                ...
                if (cHour <= 23 && cMini < 60 && cHour >= 0 && cMini >= 0) {
                    Log.v("whellView", "" + cHour + "," + cMini);
                    this.currentHour = cHour;
                    this.currentMini = cMini;
                    if (selectInf != null) {
                        selectInf.timeSelect(currentHour, currentMini);
                    }
                } else {
                    currentHour = 23;
                    currentMini = 59;
...
//    }
    ...
    public interface SelectInf {
        public void timeSelect(int hour, int minute);
    }
}
